...
class DataLoader():
    def __init__(self, train_path, val_path, test_path, args):
    ...
    def reformat_original_to_squad(self, train, val, test):
        # Reformat the answers column such that it is a ditctionary with two lists: one for the text of
        # the answer and the other one for the start position of the answer within the context
        train["answers"] = train.apply(lambda row: {"text": [row["answer"]], "answer_start": [row["answer_start"]]}, axis=1)
        val["answers"] = val.apply(lambda row: {"text": [row["answer"]], "answer_start": [row["answer_start"]]}, axis=1)
        test["answers"] = test.apply(lambda row: {"text": [row["answer"]], "answer_start": [row["answer_start"]]}, axis=1)
        return train, val, test

    def main(self):
        train, val, test = self.load_dataset_splits()
        train, val, test = self.resplit_dataset(train, val, test)
        print(self.args.original)
        if self.args.original == 'tr':
            train, val, test = self.reformat_to_squad(train, val, test)
        elif self.args.original == 'or':
            train, val, test = self.reformat_original_to_squad(train, val, test)
        return train, val, test