...
def load_dataset_splits(config):
    train = load_dataset("json", data_files=config['dataset']['train_path'])['train']
    val = load_dataset("json", data_files=config['dataset']['val_path'])['train']
    test = load_dataset("json", data_files=config['dataset']['test_path'])['train']

    train = from_dataset_to_dataframe(train)
    val = from_dataset_to_dataframe(val)
    test = from_dataset_to_dataframe(test)

    merged = pd.concat([train, val, test], axis=0, ignore_index=True)
    return merged

def plot_ner_labels(datasets, save_path):
    label_names = {
        0: "O",
        1: "B-PER",
        2: "I-PER",
    ...
    for name, df in datasets.items():
        # Flatten all ner_tags into a single list
    ...
    plt.title('NER Label Proportions Across Datasets')
...
def text_length_analysis(text_col, datasets, save_path):
    ...
    length_data = []
    for name, df in datasets.items():
        df['text_length'] = df[text_col].astype(str).apply(lambda x: len(x.split()))  # or len(x) for characters
...
filename_1 = "NER_label_proportions_across_datasets.png"
save_path_1 = os.path.join(save_dir, filename_1)
filename_2 = "NER_length_distribution.png"
save_path_2 = os.path.join(save_dir, filename_2)

plot_ner_labels(datasets, save_path_1)
